...
We calculate the center of the highlighted pixels from the *image_raw* mask and use the x position of this to determine whether or not the robot is facing directly towards the dumbbell. If it is not, we turn until the center of the pixels is approximately at the center of the image. The variance here is covered by the fact the gripper opens wider than the handle of the dumbbell. Then once we are centered, we move directly forward towards the dumbbell, correcting our angle as we get closer. Once the robot's LIDAR reading is small enough that the gripper will be around the handle of the dumbbell, we know we are close enough.
